...
const TablaGestion = ({
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg border border-blue-100 overflow-hidden">
      {/* Encabezado */}
      <div className="bg-gradient-to-r from-blue-500 to-cyan-500 text-white p-3 sm:p-4">
            ...
            <thead>
              <tr className="bg-blue-50 border-b border-blue-200">
                {columns.map((column) => (
                  <th
                    key={column.key}
                    className={`px-4 py-3 text-blue-900 font-medium text-sm ${column.align === 'center' ? 'text-center' :
                ...
                {showActions && (
                  <th className="px-4 py-3 text-center text-blue-900 font-medium text-sm">
                    Acciones
                  </th>
                )}
              </tr>
            ...
            <tbody>
              {currentData.length > 0 ? (
                currentData.map((item, index) => (
                  <tr key={item.id || index} className="hover:bg-blue-50/50 border-b border-blue-100 transition-colors">
                    {columns.map((column) => (
                      <td
                        key={column.key}
                        className={`px-4 py-4 text-sm ${column.align === 'center' ? 'text-center' :
                          column.align === 'right' ? 'text-right' : 'text-left'
